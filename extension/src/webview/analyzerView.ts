...
import { AnalysisResult, WorkspaceIndexResult } from "../../../analyzer/types"
...
function toFeatureGraphData(analysis: AnalysisResult) {
...
function toKnowledgeGraphData(analysis: AnalysisResult) {
...
export function getWorkspaceIndexHtml(
  webview: vscode.Webview,
  extensionUri: vscode.Uri,
  summary: WorkspaceIndexResult
) {
  const d3ScriptUri = webview.asWebviewUri(
    vscode.Uri.file(path.join(extensionUri.fsPath, "node_modules", "d3", "dist", "d3.min.js"))
  )
