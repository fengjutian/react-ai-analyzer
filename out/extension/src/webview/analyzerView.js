...
exports.getWorkspaceIndexHtml = getWorkspaceIndexHtml;
...
function toKnowledgeGraphData(analysis) {
    const nodeMap = new Map();
    const links = [];
    const labels = {};
    ...
    const inferKind = (name) => {
        if (componentSet.has(name))
            return "component";
        if (importedComponentSet.has(name))
        ...
        if (typeSet.has(name))
            return "type";
        if (dependencySet.has(name) || name.startsWith(".") || name.includes("/"))
            return "dependency";
        return "group";
    };
    const ensureNode = (id, kind, label) => {
        if (!nodeMap.has(id)) {
            nodeMap.set(id, { id, kind });
        }
        if (label)
...
function getWorkspaceIndexHtml(webview, extensionUri, summary) {
    const d3ScriptUri = webview.asWebviewUri(vscode.Uri.file(path.join(extensionUri.fsPath, "node_modules", "d3", "dist", "d3.min.js")));
    const nonce = getNonce();
    const graphData = (() => {
        const nodes = [];
